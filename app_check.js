...
function blankState() {
  ...
  return {
    ...
    ieltsScores: [], fltTests: [], readingSessions: [], readingErrors: [],
    ...
    resources: DB.RES_SEED.map((r, i) => ({ id: 'res' + i, ...r, rating: '', notes: '' })),
    notes: [], universities: [],
...
async function load() {
  ...
  if (loaded && typeof loaded === 'object') {
    ...
    // seeds should not duplicate if the user already has records
    if (loaded.resources) state.resources = loaded.resources;
...
function overallBand(s) {
  const vals = MODULES.map((m) => num(s[m])).filter((v) => v !== null);
...
function renderDashboard() {
  ...
  const cur = currentScores(), tgt = targets();
  const overallGap = tgt.overall - cur.overall;
  
  #dashCurrentScore.textContent = cur.overall.toFixed(1);
  #dashTargetGap.textContent = overallGap > 0 ? overallGap.toFixed(1) : '0 (Met)';
  
  // Progress bar calculation
  let progress = 0;
  if(cur.overall >= tgt.overall) progress = 100;
  else if (cur.overall < 5.0) progress = 10;
  else progress = ((cur.overall - 5.0) / (tgt.overall - 5.0)) * 100;
  ...
  let cardsHtml = '';
  let worstMod = 'reading', maxGap = -99;
  
  mods.forEach(m => {
    const c = cur[m], t = tgt[m], gap = t - c;
    if (gap > maxGap) { maxGap = gap; worstMod = m; }
    
    let statusPill = gap <= 0 ? '<span class="pill pill-ok">On Track</span>' 
                   : gap <= 0.5 ? '<span class="pill pill-med">Close</span>'
                   : '<span class="pill pill-crit">Needs Work</span>';
                   
    cardsHtml += 
      <div class="stat" onclick="window.go('+m+')" style="cursor:pointer; border-color:var(--border); transition: border-color 0.2s;" onmouseover="this.style.borderColor='var(--primary)'" onmouseout="this.style.borderColor='var(--border)'">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:0.5rem;">
          <span class="stat-label">+m+</span>
          +statusPill+
        </div>
        <div class="stat-value">+c.toFixed(1)+</div>
        <div class="stat-sub">Target: +t.toFixed(1)+</div>
      </div>
    ;
  });
  #dashModuleCards.innerHTML = cardsHtml;
  
  // Priority content
  #dashPriorityContent.innerHTML = 
    <h3 style="font-size: 1.1rem; color: var(--primary); text-transform:capitalize;">+worstMod+</h3>
    <p class="muted">+worstMod+ is currently your largest target gap (+maxGap.toFixed(1)+ band points). Focus your session here today.</p>
    <button class="btn btn-primary" style="align-self: flex-start;" onclick="window.go('+worstMod+')">START +worstMod.toUpperCase()+ SESSION</button>
  ;
  
  // Checklist
  buildPlan();
  const day = state.dailyTasks[today()] || [];
  if(day.length === 0) {
    #dashChecklist.innerHTML = '<p class="empty-inline">No tasks today.</p>';
  } else {
    #dashChecklist.innerHTML = day.slice(0, 5).map((t, idx) => 
      <li class="task +(t.done?'is-done':'')+">
        <input type="checkbox" +(t.done?'checked':'')+ onclick="toggleTask('+today()+', +idx+, true)">
        <div class="task-main">
...
function renderIelts() {
  ...
  $('#gapTable').innerHTML = MODULES.map((m) => {
    ...
    return '<div class="target-row"><span class="t-name">' + MOD_LABEL[m] + '</span>' +
      '<span class="t-val">' + g.cur.toFixed(1) + ' → ' + g.tgt.toFixed(1) + '</span>' +
    ...
    cur.overall.toFixed(1) + ' → ' + tgt.overall.toFixed(1) + '</span><span class="grow muted small">estimated from your logs</span></div>';
...
function calcBand() {
  ...
  if (!f) return;
  const vals = {};
  MODULES.forEach((m) => { vals[m] = num(f.elements[m].value); });
  const o = overallBand(vals);
  $('#calcOut').textContent = o == null ? '—' : o.toFixed(1);
}
...
function resourceCard(r) {
  return '<article class="res-card' + (r.status === 'Completed' ? ' is-done' : '') + '">' +
    ...
    '<div class="res-actions">' +
      '<a class="btn btn-sm btn-primary" href="' + esc(r.url) + '" target="_blank" rel="noopener">Open</a>' +
...
function renderResources() {
  const cats = ['All', ...new Set(state.resources.map((r) => r.category).filter(Boolean))];
  $('#resFilters').innerHTML = cats.map((c) =>
    '<button class="word-chip' + (resFilter.all === c ? ' is-on' : '') + '" data-resfilter="' + esc(c) + '">' + esc(c) + '</button>').join('');
  let rows = getList('resources');
  if (resFilter.all !== 'All') rows = rows.filter((r) => r.category === resFilter.all);
...
function renderTeacher() {
  ...
  const cards = [
    ['Primary Focus', '<p style="font-size:1.1rem; color:var(--primary); font-weight:600;">' + esc(t.focus.title) + '</p><p>' + esc(t.why) + '</p>'],
    ['Actionable Task', '<p>' + esc(t.task) + '</p><p class="muted small">Class today: ' + esc(t.classToday) + '</p>'],
    ['Quick Tip', '<p>' + esc(t.tip) + '</p>']
  ];
  #teacherBoard.innerHTML = cards.map((c) =>
    '<article class="card"><h3>' + esc(c[0]) + '</h3>' + c[1] + '</article>').join('');
...
function parseImport(text) {
  ...
  const seen = new Set();
  lines.forEach((line) => {
    const m = line.match(/^([A-Za-zÄÖÜäöüß][A-Za-zÄÖÜäöüß'’\- ]{1,28})\s*[–—\-:=]\s*(.{3,90})$/);
    if (!m) return;
    ...
    seen.add(key);
    const german = /[ÄÖÜäöüß]/.test(word) || /^(der|die|das)\s/i.test(word);
    out.push({
...
/* =========================================================
   ...
   ========================================================= */
const RENDERERS = {
  dashboard: renderDashboard, today: renderToday, ielts: renderIelts, reading: renderReading,
...
async function initBackground() {
  ...
  try { await loadScript('https://cdnjs.cloudflare.com/ajax/libs/three.js/0.128.0/three.min.js'); }
...
function bindEvents() {
  ...
  document.addEventListener('input', (e) => {
    ...
    if (f) { filters[f.dataset.filter] = f.value; return renderList(f.dataset.filter); }
...
function initQuickLog() {
  ...
  ('.ql-btn').forEach(b => {
    b.addEventListener('click', (e) => {
      ...
      const map = {
        ...
        'germanLesson': 'german' // actually this might need special handling
      };
      
      currentQlTable = map[type];
      if(!currentQlTable) { toast('Coming soon', 'warn'); return; }
      
      if(currentQlTable === 'german') {
         // handle german lesson special
         modal.hidden = true;
         go('german');
         return;
      }
      ...
      currentQlSchema.fields.forEach(f => {
         ...
         d.innerHTML = fieldHtml(f, currentQlTable, null);
  ...
  form.addEventListener('submit', (e) => {
    ...
    currentQlSchema.fields.forEach(f => {
      ...
      else if (f.type === 'number' || f.type === 'band') {
        const n = parseFloat(v);
        obj[f.key] = isNaN(n) ? null : n;
      } else {
        obj[f.key] = v;
      }
    });
    ...
    for (const f of currentQlSchema.fields) {
      if (f.required && (obj[f.key] === null || obj[f.key] === '')) {
        toast(f.label + ' is required', 'bad');
        return;
      }
    }
    
    // Sync hooks for single source of truth
    syncHooks(currentQlTable, obj);
    
    state[currentQlTable].push(obj);
    if(currentQlSchema.sort) {
      state[currentQlTable].sort((a,b) => (a[currentQlSchema.sort] < b[currentQlSchema.sort] ? 1 : -1));
    }
...
function syncHooks(table, obj) {
  // If we log an FLT, also add an IELTS score if bands are present
  if (table === 'fltTests' && obj.listening && obj.reading) {
    const score = {
      id: uid(),
      date: obj.date || iso(new Date()),
      test: 'FLT ' + (obj.number || ''),
